...
from FSI_DC import assembly_B_ID

# NO SOLID INERTIA - VERSION

def solve(lx_f, ly_f, nx_f, ny_f, dx_f, dy_f, Nt, vel0_f, lx_s, ly_s, nx_s, ny_s, dx_s, dy_s, d0_x_s, d0_y_s, mu_s, lam_s, dt, Nb, tol=1e-6, minit=0, maxit=100): 
    ...
    # Valuta se mettere il controllo t>0 o no.. non è detto che serva
        while err > tol and k < maxit or k < minit:
            un = u_s + dt * v_s + dt * dt * (0.5 - beta) * a_s
            vn = v_s + dt * (1.0 - gamma) * a_s
...
if __name__ == "__main__":
    ...
    u_f, v_f, p_f, u_s, v_s, P, T = solve(lx_f, ly_f, nx_f, ny_f, dx_f, dy_f, Nt, vel0_f, lx_s, ly_s, nx_s, ny_s, dx_s, dy_s, d0_x_s, d0_y_s, mu_s, lam_s, dt, Nb, tol, minit=0, maxit=100)
