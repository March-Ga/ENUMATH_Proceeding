...
from FSI_DC import solid_assembly
from FSI_DC import fluid_assembly
from FSI_DC import fluid_solver
from FSI_DC import fluid_solver_Pen
from FSI_DC import assembly_B_ID_Pen
from FSI_DC import assembly_indicator

# NO SOLID INERTIA - VERSION

def solve(lx_f, ly_f, nx_f, ny_f, dx_f, dy_f, Nt, vel0_f, lx_s, ly_s, nx_s, ny_s, dx_s, dy_s, d0_x_s, d0_y_s, mu_s, lam_s, dt, Nb, tol=1e-6, minit=0, maxit=100, mu=1e5): 
    ...
    A_pen = A_s + mu/dt * M_el
    # C = M.sum(axis=1).A1
    # CTC = C**2
    # A_s_pen = A_s + mu * dt * lil_matrix(np.diag(np.concatenate((np.ones(Nb), np.ones(Nb)))))
    ...
    for t in range(Nt):
        ...
        Bx, By = assembly_B_ID_Pen(u1_s + P[0,:] + d0_x_s, u2_s + P[1,:] + d0_y_s, dx_f, dy_f, nx_s, ny_s, nx_f, ny_f, T) 
        Ix, Iy = assembly_indicator(u1_s + P[0,:] + d0_x_s, u2_s + P[1,:] + d0_y_s, dx_f, dy_f, nx_s, ny_s, nx_f, ny_f, lx_f, ly_f, T) 
        # print(Ix.shape)
        # print(Iy.shape)
    ...
    # Valuta se mettere il controllo t>0 o no.. non è detto che serva
        while err > tol and k < maxit or k < minit:
            ...
            vf_x = vf_x.reshape((ny_f, nx_f + 1))  
            vf_y = vf_y.reshape((ny_f + 1, nx_f))

            # print(F_x.shape, F_y.shape) 
            u_f, v_f, p_f = fluid_solver_Pen(usol_f[t].copy(), vsol_f[t].copy(), nx_f, ny_f, dx_f, dy_f, dt, nu_f, vf_x, vf_y, Af, vel0_f, mu, Ix.T, Iy.T)
            # u_f, v_f, p_f = fluid_solver(usol_f[t].copy(), vsol_f[t].copy(), nx_f, ny_f, dx_f, dy_f, dt, nu_f, F_x, F_y, Af, vel0_f)
            u_f_small = u_f[1:-1, 1:]
            v_f_small = v_f[1:, 1:-1]

            # err = np.linalg.norm(lam - lam_old, ord=2) / np.linalg.norm(lam, ord=2)
        ...
        # CHECK INTERFACE CONDITION ✅
            # errx = np.linalg.norm((u_s[:Nb]-usol_s[t][:Nb]) / dt - (Bx @ u_f_small.ravel()) , ord=2)
            # print('errx', errx)
            # erry = np.linalg.norm((u_s[Nb:]-usol_s[t][Nb:]) / dt - (By @ v_f_small.ravel()) , ord=2)
            # print('erry', erry)
            # print(k, err)
        print(k, err)
    ...
    return usol_f, vsol_f, psol_f, usol_s, vsol_s, P, T

if __name__ == "__main__":
    file_base = "test_simulation" 
    folder = "DATA"
    ...
    u_f, v_f, p_f, u_s, v_s, P, T = solve(lx_f, ly_f, nx_f, ny_f, dx_f, dy_f, Nt, vel0_f, lx_s, ly_s, nx_s, ny_s, dx_s, dy_s, d0_x_s, d0_y_s, mu_s, lam_s, dt, Nb, tol, minit=0, maxit=100, mu=1e5)
