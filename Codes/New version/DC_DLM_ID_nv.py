...
def solve(lx_f, ly_f, nx_f, ny_f, dx_f, dy_f, Nt, vel0_f, lx_s, ly_s, nx_s, ny_s, dx_s, dy_s, d0_x_s, d0_y_s, mu_s, lam_s, dt, Nb, nu_f, rho_f, rho_s, tol=1e-6, minit=0, maxit=100): 
    ...
    # Valuta se mettere il controllo t>0 o no.. non è detto che serva
        ...
        # lam_x = M @ lam_x
        # lam_y = M @ lam_y
        # plt.quiver(lam_x.reshape(nx_s + 1, ny_s + 1), lam_y.reshape(nx_s + 1, ny_s + 1))
        # plt.show()
        print(k,err)
        if t % 100 == 0:
            print(f"Time step: {t}")
...
if __name__ == "__main__":
    ...
    u_f, v_f, p_f, u_s, v_s, P, T = solve(lx_f, ly_f, nx_f, ny_f, dx_f, dy_f, Nt, vel0_f, lx_s, ly_s, nx_s, ny_s, dx_s, dy_s, d0_x_s, d0_y_s, mu_s, lam_s, dt, Nb, nu_f, rho_f, rho_s, tol, minit=0, maxit=100)
